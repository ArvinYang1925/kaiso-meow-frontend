...
const AICouponsGeneratorPage = () => {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    if (
      !data.numberOfPhases ||
      data.numberOfPhases < 1 ||
      data.numberOfPhases > 6
    ) {
    ...
    const requestData: AiCouponGenerateRequest = {
      ...
      ...(data.keywordThemes?.trim() && {
        keywordThemes: data.keywordThemes.trim(),
      }),
      ...(data.phaseDurationDays &&
        data.phaseDurationDays > 0 && {
          phaseDurationDays: Number(data.phaseDurationDays),
        }),
    };
  ...
  const validateDateFormat = (value: string) => {
    ...
    if (eightDigitPattern.test(value)) {
      ...
      setError("launchDate", {
        type: "manual",
        message:
          "建議使用標準日期格式 (YYYY-MM-DD)，例如：2025-07-13、20250713 或點擊日曆圖標選擇日期",
      });
      return;
    }
    ...
    <div className="py-4 md:py-8 w-full max-w-full overflow-hidden">
      <div className="md:container md:mx-auto md:px-6 lg:px-8 pb-20 md:pb-8">
        ...
        <div className="mb-6 px-4 md:px-0">
          <h1 className="text-xl font-bold mb-2">智慧建立折扣碼計劃</h1>
                ...
                >
                  ...
                  <div className="space-y-2 w-full max-w-full">
                    ...
                    <div className="relative flex gap-2">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          if (e.target.value) {
                            setValue("launchDate", e.target.value, {
                              shouldValidate: true,
                            });
                          }
                        }}
                    ...
                    <p className="text-xs sm:text-sm text-gray-500 break-words">
                      建議使用標準格式
                      (YYYY-MM-DD，如：2025-07-13)，或點擊日曆圖標選擇日期。設定課程上架日期有助於
                      AI 規劃合適的促銷時程。
                    </p>
                  </div>
                  ...
                  <div className="space-y-2 w-full max-w-full">
                    ...
                    {errors.numberOfPhases && (
                      <p className="text-sm text-red-500 break-words">
                        {errors.numberOfPhases.message}
                      </p>
                    )}
                    <p className="text-xs sm:text-sm text-gray-500 break-words">
                      建議設定 2-4 種方案，創造階段性的促銷效果。最多可設定 6
                      種方案。
                    </p>
                  </div>
                  ...
                  <div className="space-y-2 w-full max-w-full">
                    ...
                    {errors.phaseDurationDays && (
                      <p className="text-sm text-red-500 break-words">
                        {errors.phaseDurationDays.message}
                      </p>
                    )}
                    <p className="text-xs sm:text-sm text-gray-500 break-words">
