...
export default function CoursesDetailPage() {
  ...
  useEffect(() => {
    if (isEditMode) {
      ...
      const mockCourseData = {
        title: "React 入門課程",
        subtitle: "從零開始學習 React",
        description: "這是一個 React 入門課程...",
        price: "1999",
      };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // TODO: 實現課程創建/更新邏輯
      console.log(isEditMode ? "更新課程:" : "創建課程:", formData);
      navigate(ADMIN_ROUTES.COURSES);
    } catch (error) {
      console.error(isEditMode ? "更新課程失敗:" : "創建課程失敗:", error);
    } finally {
      setIsLoading(false);
    }
  };
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex justify-end space-x-4">
                  ...
                  <Button type="submit" disabled={isLoading}>
                    {isLoading ? (isEditMode ? "更新中..." : "建立中...") : (isEditMode ? "更新課程" : "建立課程")}
                  </Button>
                </div>
              </form>
            </CardContent>
