...
import { useImageWithFallback, DEFAULT_COURSE_COVER } from "@/components/utils/courseImageUtils";
...
export default function CourseManagementDetailPage() {
  ...
  useEffect(() => {
    if (currentCourse && !isFormInitialized) {
      reset({
        ...
        duration: currentCourse.duration ? currentCourse.duration.toString() : "",
  ...
  const onSubmit = async (data: CourseDetailFormData) => {
    ...
    try {
      ...
      if (data.courseType === "paid" && (!data.price || parseFloat(data.price) <= 0)) {
      ...
      const courseData: UpdateCourseModel = {
        ...
        coverUrl: uploadedCoverUrl || currentCourse?.coverUrl || DEFAULT_COURSE_COVER
      };
  ...
  // 檢查是否有自定義圖片（非預設圖片）
  const hasCustomImage = selectedFile || uploadedCoverUrl || (coverPreview && coverPreview !== DEFAULT_COURSE_COVER);
            ...
            <div className="bg-white rounded-lg border border-slate-200">
              <div className="p-6 space-y-6">
                ...
                <div className="space-y-1">
                  ...
                  />
                  {errors.highlight && (
                    <p className="text-sm text-red-500">{errors.highlight.message}</p>
                  )}
                  <p className="text-sm text-gray-500">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-gray-500">
                    例如，對於10小時30分鐘，輸入 10.5。若留空，系統將自動計算課程中所有單元的總學時數。
                  </p>
                </div>
                ...
                <div className="flex justify-end space-x-4">
                  ...
                  >
                    {isUpdating ? "更新中..." : isUploading ? "上傳中..." : "更新課程"}
                  </Button>
                </div>
              </div>
            </div>
                    ...
                    {hasPreview ? (
                      ...
                      <div className="relative w-full h-full">
                        ...
                        <div className="absolute bottom-2 left-2 right-2 bg-black bg-opacity-70 text-white text-xs p-2 rounded opacity-0 group-hover:opacity-100 transition-opacity">
                          {hasCustomImage ? "點擊或拖拽以替換圖片" : "點擊或拖拽以上傳自定義圖片"}
                        </div>
                      </div>
                    ) : (
                      // 上傳提示模式
