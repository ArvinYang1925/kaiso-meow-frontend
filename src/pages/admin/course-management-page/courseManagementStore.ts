...
import { getCourseCoverUrl, DEFAULT_COURSE_COVER } from "@/components/utils/courseImageUtils";
import {
  createCourse,
  getCourseDetail,
  updateCourse,
...
// 排序選項類型
type CourseSortOption = "created_desc" | "created_asc" | "student_count_desc" | "student_count_asc" | "title_asc" | "title_desc";
...
interface CourseActions {
  ...
  updateCourseDetail: (courseId: string, data: UpdateCourseModel) => Promise<boolean>;
  removeCourse: (courseId: string) => Promise<boolean>;
  
  // 課程狀態操作
  togglePublishCourse: (courseId: string, isPublished: boolean) => Promise<boolean>;
 ...
 * 2. 建立時間（新 → 舊）
 */
const sortCoursesFixed = (courses: CourseListItemModel[]): CourseListItemModel[] => {
...
const loadAllCourses = async (): Promise<CourseListItemModel[]> => {
  try {
    ...
    throw new Error(`無法載入課程資料: ${error instanceof Error ? error.message : '未知錯誤'}`);
  }
};
  ...
  immer<CourseState & CourseActions>((set, get) => ({
    ...
    removeCourse: async (courseId) => {
      try {
        ...
        handleErrorMessage("刪除失敗", "無法刪除課程，請稍後再試。");
        return false;
      } finally {
        set((state) => {
    ...
    togglePublishCourse: async (courseId, isPublished) => {
      try {
        const response = await toggleCoursePublishStatus(courseId, { isPublished });
        ...
        if (response.status === "success") {
          ...
          set((state) => {
            // 更新 allCourses 中的狀態
            const allCourseIndex = state.allCourses.findIndex(course => course.id === courseId);
            if (allCourseIndex !== -1) {
