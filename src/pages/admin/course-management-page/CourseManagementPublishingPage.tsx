...
export default function CoursePublishingManagementPage() {
  ...
  const handlePublishToggle = async (checked: boolean) => {
    ...
    try {
      console.log("Publishing toggle:", { courseId, checked });
  ...
  const handleDeleteCourse = async () => {
    if (!courseId) {
      console.error("No courseId found for deletion");
      toast({
        variant: "destructive",
        title: "刪除失敗",
        description: "無效的課程 ID",
    ...
    try {
      console.log("UI: Starting course deletion process for ID:", courseId);
      ...
      console.log("UI: Course deletion result:", success);

      if (success) {
        // 成功刪除，顯示成功訊息並導航
        console.log("UI: Course deletion successful, preparing to navigate");
        ...
        // 稍微延遲導航，確保用戶看到成功訊息
        setTimeout(() => {
          console.log("UI: Navigating to courses list");
          navigate(ADMIN_ROUTES.COURSES);
        }, 1500);
      } else {
        // 刪除失敗，但 store 應該已經顯示錯誤訊息
        console.error("UI: Course deletion failed");

        // 作為備用，如果 store 沒有顯示錯誤訊息，這裡再顯示一次
        setTimeout(() => {
          toast({
            variant: "destructive",
            title: "刪除失敗",
